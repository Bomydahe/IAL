...
void Stack_Init( Stack *stack ) { 
	...
	if (stack == NULL) //V pripade, že funkcia dostane ako parametr stack == NULL, nastane SERR_INIT   
		Stack_Error(SERR_INIT);
		else stack->topIndex = -1; //prazdnemu zasobniku nastavim hodnotu top indexu na -1
...
int Stack_IsEmpty( const Stack *stack ) {
    ...
    return stack->topIndex == -1; //zasobnik je prazdny ak top index obsahuje hodnotu -1
}
...
int Stack_IsFull( const Stack *stack ) {
    ...
    return stack->topIndex == STACK_SIZE -1; //zasobnik nemoze obsahovat viac ako STACK_SIZE prvkov
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	if (!Stack_IsEmpty(stack)){ // overenie prazdnosti zasobnika
		*dataPtr = stack->array[stack->topIndex];
		return;} //priradi hodnotu z vrcholu zasobniku do dataPtr
		
	else Stack_Error(SERR_TOP);
...
void Stack_Pop( Stack *stack ) {
    ...
    if (!Stack_IsEmpty(stack))	// overenie prazdnosti zasobnika
		stack->topIndex--; //odstrani prvkok z vrcholu zasobiku
...
void Stack_Push( Stack *stack, char data ) {
    ...
    if (!Stack_IsFull(stack)){ 	// overenie prazdnosti zasobnika
		stack->topIndex++;
		stack->array[stack->topIndex] = data; // priradenie prvku na poziciu top indexu
		}
	else Stack_Error(SERR_PUSH);
}
